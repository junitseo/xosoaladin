...
import styles from '@/styles/pages/SoDauDuoi/MienTrung/index.module.scss'
import Head from 'next/head'
import parse from 'html-react-parser'
import replaceDashFromDate from '@/helpers/replaceDashFromDate'

const SoDauDuoiMienTrung = ({ tags }: { tags: any }) => {
            ...
            if (!date) {
                ...
                const xosoMore: IDataXosoMore[] = await Promise.all(
                    arrayOfDate.map(async (item) => {
                        const returnResult = await getKqxsmt(item)
    ...
    return (
        <>
            <Head>
                <link
                    rel="canonical"
                    href="https://xosoaladin.com/so-dau-duoi/mien-trung"
                />
                {tags?.map((tag, index) => (
            ...
            <div className={styles.mienTrung}>
                <h2 className={styles.title}>Sớ đầu đuôi miền Trung</h2>
...
export default SoDauDuoiMienTrung

export async function getServerSideProps() {
    const params: any = {
        link: '/so-dau-duoi/mien-trung',
    }
